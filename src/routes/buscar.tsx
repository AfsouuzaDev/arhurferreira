...
function SearchView() {
  ...
  const binaryDisabledReason =
    mode === "substring"
      ...
      : !isSorted
      ? "Acesso Negado: A Busca Binária requer que o vetor esteja previamente ordenado pela chave de busca."
      : undefined;
  ...
  // Comparativo empírico (mesma busca, ambos algoritmos)
  const empirical = useMemo(() => {
    if (!term.trim() || sample.length === 0) return null;
    const lin = dryRunLinear(sample, mode, term);
    const bin = mode === "substring" || !isSorted
      ? { comparisons: 0, hits: 0, feasible: false }
      : dryRunBinary(sample, key, term);
    return { lin, bin };
  }, [sample, term, mode, key, isSorted]);

  const handlePlay = () => {
    if (!term.trim() || sample.length === 0) return;
    engine.run(algo, mode, key, term);
  };
  const handleReset = () => { engine.reset(sample); setTerm(""); };
    ...
    <div className="space-y-4">
      <div className="flex gap-2">
        <button
          ...
          className={`clip-cyber-sm border px-4 py-1.5 text-xs uppercase font-bold ${subTab === "viz" ? "border-neon-yellow bg-neon-yellow/15 text-neon-yellow" : "border-neon-cyan/60 text-neon-cyan hover:bg-neon-cyan/10"}`}
        >▸ Simulação Visual</button>
        <button
          onClick={() => setSubTab("perf")}
          className={`clip-cyber-sm border px-4 py-1.5 text-xs uppercase font-bold ${subTab === "perf" ? "border-neon-yellow bg-neon-yellow/15 text-neon-yellow" : "border-neon-cyan/60 text-neon-cyan hover:bg-neon-cyan/10"}`}
          ...
          <div className="space-y-3">
            ...
            {empirical && (
              <div className="panel-cyber clip-cyber p-3">
                <div className="text-[10px] text-neon-magenta mb-2 uppercase tracking-widest">
                  ▸ Comparativo Empírico — alvo "{term}" · n={sample.length}
                </div>
                <div className="grid grid-cols-2 gap-2 text-center">
                  <div className="border border-neon-cyan/40 clip-cyber-sm p-2">
                    <div className="text-[9px] text-neon-cyan">BUSCA LINEAR O(n)</div>
                    <div className="text-2xl text-neon-magenta font-bold tabular-nums">{empirical.lin.comparisons}</div>
                    <div className="text-[9px] text-foreground/70">comparações · {empirical.lin.hits} hit(s)</div>
                  </div>
                  <div className="border border-neon-cyan/40 clip-cyber-sm p-2">
                    <div className="text-[9px] text-neon-cyan">BUSCA BINÁRIA O(log n)</div>
                    {empirical.bin.feasible ? (
                      <>
                        <div className="text-2xl text-neon-green font-bold tabular-nums">{empirical.bin.comparisons}</div>
                        <div className="text-[9px] text-foreground/70">comparações · {empirical.bin.hits} hit(s)</div>
                      </>
                    ) : (
                      <div className="text-[10px] text-neon-magenta/80 py-2">N/A — pré-condição não satisfeita</div>
                    )}
                  </div>
                </div>
                {empirical.bin.feasible && empirical.bin.comparisons > 0 && (
                  <div className="text-[10px] text-neon-yellow/80 mt-2 text-center">
                    ⚡ Binária foi {(empirical.lin.comparisons / Math.max(1, empirical.bin.comparisons)).toFixed(1)}× mais econômica em comparações.
                  </div>
                )}
              </div>
            )}
