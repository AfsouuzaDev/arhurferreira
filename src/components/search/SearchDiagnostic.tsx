import { SEARCH_ALGORITHMS, SEARCH_INFO, type SearchAlgoKey } from "@/lib/search/algorithms";
...
export function SearchDiagnostic({ algo, comparisons, foundCount, activeLine, logs }: Props) {
  const info = SEARCH_INFO[algo];
  return (
    <div className="space-y-3">
      <Frame title="Descrição Teórica">
        <h2 className="text-base text-neon-yellow mb-2">{SEARCH_ALGORITHMS[algo].name}</h2>
        <p className="text-xs text-foreground/80 leading-relaxed font-sans">{info.description}</p>
      </Frame>

      <Frame title="Métricas">
        <div className="grid grid-cols-2 gap-2 text-center">
          <div className="border border-neon-cyan/40 p-2 clip-cyber-sm">
            <div className="text-[9px] text-neon-cyan">COMPARAÇÕES</div>
            <div className="text-2xl text-neon-yellow font-bold tabular-nums">{comparisons}</div>
          </div>
          <div className="border border-neon-cyan/40 p-2 clip-cyber-sm">
            <div className="text-[9px] text-neon-cyan">ALVOS HIT</div>
            <div className="text-2xl text-neon-green font-bold tabular-nums">{foundCount}</div>
          </div>
        </div>
      </Frame>
